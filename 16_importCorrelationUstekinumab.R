# Name: 16_importCorrelationUstekinumab.R
# Auth: umar.niazi@kcl.ac.uk
# Date: 21/12/2017
# Desc: imports the data and formats it for correlation analysis for treatment 2
...
xyplot(relativePASI ~ Rd.score | Cell.type:Transcription.factor:Stimulation, data=dfData, type=c('g', 'p', 'r'),
...
xyplot(relativePASI ~ Rd.score | Cell.type:Transcription.factor:Stimulation, data=dfData, type=c('smooth'),
       ##index.cond = function(x,y) coef(lm(y ~ x))[1], aspect='xy',# layout=c(8,2),
       par.strip.text=list(cex=0.7), scales = list(x=list(rot=45, cex=0.5)), pch=20, groups=Visit..Week.,
       auto.key=list(columns=4))

xyplot(Rd.score ~ relativePASI | Cell.type:Transcription.factor:Stimulation, data=dfData, type=c('smooth'),
       ##index.cond = function(x,y) coef(lm(y ~ x))[1], aspect='xy',# layout=c(8,2),
       par.strip.text=list(cex=0.7), scales = list(x=list(rot=45, cex=0.5)), pch=20, groups=Visit..Week.,
       auto.key=list(columns=4))
...
xyplot(Rd.score ~ relativePASI, data=dfData, type=c('smooth'),
       ##index.cond = function(x,y) coef(lm(y ~ x))[1], aspect='xy',# layout=c(8,2),
       par.strip.text=list(cex=0.7), scales = list(x=list(rot=45, cex=0.5)), pch=20, groups=Visit..Week.,
       auto.key=list(columns=4))

################################ need to confirm before applying this filter, may need repetition
####### apply the cutoffs
...
xyplot(Rd.score ~ relativePASI | Cell.type:Transcription.factor:Stimulation, data=dfData, type=c('smooth'),
       ##index.cond = function(x,y) coef(lm(y ~ x))[1], aspect='xy',# layout=c(8,2),
       par.strip.text=list(cex=0.7), scales = list(x=list(rot=45, cex=0.5)), pch=20, groups=Visit..Week.,
       auto.key=list(columns=4), ylim=c(-0.5, 1.5))

xyplot(relativePASI ~ Rd.score | Cell.type:Transcription.factor:Stimulation, data=dfData, type=c('smooth'),
       ##index.cond = function(x,y) coef(lm(y ~ x))[1], aspect='xy',# layout=c(8,2),
       par.strip.text=list(cex=0.7), scales = list(x=list(rot=45, cex=0.5)), pch=20, groups=Visit..Week.,
       auto.key=list(columns=4), xlim=c(-0.5, 1.5))

xyplot(relativePASI ~ Rd.score | Cell.type:Transcription.factor:Stimulation, data=dfData, type=c('r'),
       ##index.cond = function(x,y) coef(lm(y ~ x))[1], aspect='xy',# layout=c(8,2),
       par.strip.text=list(cex=0.7), scales = list(x=list(rot=45, cex=0.5)), pch=20, groups=Visit..Week.,
       auto.key=list(columns=4), xlim=c(-0.5, 1.5))
...
write.csv(dfData, file='dataExternal/healthyData/correlationDataUstekinumab.csv', row.names = F)
