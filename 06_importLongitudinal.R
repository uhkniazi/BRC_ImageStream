# Name: 06_importLongitudinal.R
# Auth: umar.niazi@kcl.ac.uk
# Date: 05/10/2017
# Desc: imports the longitudinal data for diseased
...
hist(x)
quantile(x, 0:20/20)
cut.pts = cut(x, breaks = quantile(x, probs = 0:20/20), include.lowest = T, labels = 0:19)
...
## choose a minimum cutoff of 13ish cells
...
xyplot(Rd.score ~ time | fModule, data=dfData, type=c('g', 'p', 'r'),
       ##index.cond = function(x,y) coef(lm(y ~ x))[1], aspect='xy',# layout=c(8,2),
       par.strip.text=list(cex=0.7), scales = list(x=list(rot=45, cex=0.5)), groups=dfData$Treatment,
       auto.key = list(columns=2))
