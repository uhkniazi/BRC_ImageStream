# Name: 20_pasi90vsRDscoreUstekinumab.R
...
write.csv(dfData, file='dataExternal/healthyData/pasi90VsRdScoreUstekinumab.csv', row.names = F)
...
write.csv(dfResults, file='Results/pasi90vsRdScoreUstekinumab.csv', row.names = F)
...
bwplot(PASI90 ~ Rd.score | fBlock, data=dfData, panel=function(x, y, ...) panel.bwplot(x, y, pch='|',...),
        par.strip.text=list(cex=0.6), varwidth=T)
...
dotplot(PASI90 ~ m+s1+s2 | cells:stimulation:time, data=d, panel=llines(d$s1, d$s2), cex=0.6, pch=20,
        par.strip.text=list(cex=0.6), main='34 Fitted Coefficients for Rd.score ~ PASI90', xlab='')
