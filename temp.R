...
gSigmaRan1 = 5
gInterceptRan1 = rnorm(n = 5, mean = 0, sd = gSigmaRan1)

gSigmaRan2 = 2
gInterceptRan2 = rnorm(n = 5, mean = 0, sd = gSigmaRan2)


## At the Population level or the second level of the model
## the intercept is 3
gPopulationIntercept = 10
## Varying intercepts
...
gGroupIntercept1  = rep(gPopulationIntercept + gInterceptRan1, 12*2)
gGroupIntercept2  = rep(gPopulationIntercept + gInterceptRan2, 12*2)

## Cluster indicator
## this is a mapping variable - used for tracking the population intercepts
iClusterIndicator1 = rep(1:5, times = 12*2)
iClusterIndicator2 = rep(1:5, times = 12*2)
## population level error, how much
## each data point varies
gSigmaPop = 5
# we have 5 groups X 6 per group = 30 observations
iErrors = rnorm(n = 120, mean = 0, sd = gSigmaPop)

## We generate some random predictor and
## generate some data
## predictor variable
gPredictor = runif(n = 120, min = 0, max = 20)
## Population slope.
...
gSlope = 1.5
## Outcome
gResponse = gGroupIntercept1 + gGroupIntercept2 + gSlope*gPredictor + iErrors

dfData = data.frame(resp = gResponse, pred=gPredictor, group1=factor(iClusterIndicator1), groupIndex1=iClusterIndicator1,
                    group2=factor(iClusterIndicator2), groupIndex2=iClusterIndicator2)


library(lattice)
xyplot(resp ~ pred | group1, data=dfData, type=c('g', 'p', 'r'), 
       index.cond = function(x,y) coef(lm(y ~ x))[1], aspect='xy',# layout=c(8,2), 
       par.strip.text=list(cex=0.7))

xyplot(resp ~ pred | group2, data=dfData, type=c('g', 'p', 'r'), 
       index.cond = function(x,y) coef(lm(y ~ x))[1], aspect='xy',# layout=c(8,2), 
       par.strip.text=list(cex=0.7))
...
library(lme4)
fit.lme = lmer(resp ~ pred + (1 | group1) + (1 | group2), data=dfData, REML=F)
summary(fit.lme)
...
stanDso = rstan::stan_model(file='normResponseRandomEffects2.stan')
...
l = gammaShRaFromModeSD(sd(dfData$resp), 2*sd(dfData$resp))


m = model.matrix(resp ~ pred, data=dfData)

lStanData = list(Ntotal=nrow(dfData), Nclusters1=nlevels(dfData$group1), Nclusters2=nlevels(dfData$group2), 
                 NgroupMap1=dfData$groupIndex1, NgroupMap2=dfData$groupIndex2, Ncol=ncol(m), X=m,
                 y=dfData$resp, gammaShape=l$shape, gammaRate=l$rate)

fit.stan = sampling(stanDso, data=lStanData, iter=10000, chains=4, pars=c('betas', 'sigmaRan1', 'sigmaRan2',
...
stanDso = rstan::stan_model(file='normResponseRandomEffects.stan')

lStanData = list(N=nrow(dfData), rt=dfData$resp, so=dfData$pred, J=nlevels(dfData$group1), K=nlevels(dfData$group2), 
                 subj=dfData$groupIndex1, item=dfData$groupIndex2)

fit.stan2 = sampling(stanDso, data=lStanData, iter=2000, chains=2, pars=c('beta', 'sigma_e', 'sigma_u',
                                                                         'sigma_w', 'u', 'w'),
                    cores=2)
print(fit.stan, digits=3)
