...
dim(dfPatient)
colnames(dfPatient)
cn = c("Patient.ID", "Treatment", "Visit..Week.", "Transcription.factor", "Stimulation", "Cell.type" , "Rd.score", "Cell.count", "Baseline.PASI", "Week.1.PASI", "Week.4.PASI", "Week.12.PASI",  "Gender", "Age", "Ethnicity", "PsA", "Weight.baseline..Kg.", "ADL.conc", "USK.conc", "relativePASI", "PASI90", "PASI75", "PASI50")
length(cn)
dfData = dfPatient[,cn]

## check structure of data
str(dfData)
dfData = dfData[dfData$Treatment == 'Ustekinumab',]
...
## this should drop the unstimulated as they do not have a rd-score
dfData = droplevels.data.frame(dfData)
...
xyplot(Rd.score ~ time | Cell.type:Transcription.factor:Stimulation, data=dfData, type=c('g', 'p', 'r'),
...
xyplot(Rd.score ~ time | Cell.type:Transcription.factor:Stimulation, data=dfData, type=c('smooth'),
       ##index.cond = function(x,y) coef(lm(y ~ x))[1], aspect='xy',# layout=c(8,2),
       par.strip.text=list(cex=0.7), scales = list(x=list(rot=45, cex=0.5)))

## population level effect
xyplot(Rd.score ~ time, data=dfData, type=c('smooth'),
       ##index.cond = function(x,y) coef(lm(y ~ x))[1], aspect='xy',# layout=c(8,2),
       par.strip.text=list(cex=0.7), scales = list(x=list(rot=45, cex=0.5)))

## time as categorical variable
xyplot(Rd.score ~ Visit..Week. | Cell.type:Transcription.factor:Stimulation, data=dfData, type=c('r', 'g', 'p'), pch=19, cex=0.6,
       #index.cond = function(x,y) coef(lm(y ~ x))[1], aspect='xy',# layout=c(8,2), 
       par.strip.text=list(cex=0.7), scales = list(x=list(rot=45, cex=0.5)))
...
## time as categorical variable
xyplot(Rd.score ~ Visit..Week. | fModule, data=dfData, type=c('r', 'g', 'p'), pch=19, cex=0.6,
       #index.cond = function(x,y) coef(lm(y ~ x))[1], aspect='xy',# layout=c(8,2), 
       par.strip.text=list(cex=0.7), scales = list(x=list(rot=45, cex=0.5)))

xyplot(Rd.score ~ Visit..Week. | fModule, data=dfData, type=c('smooth'), pch=19, cex=0.6,
       #index.cond = function(x,y) coef(lm(y ~ x))[1], aspect='xy',# layout=c(8,2), 
       par.strip.text=list(cex=0.7), scales = list(x=list(rot=45, cex=0.5)))
...
yresp = density(ivResp)
plot(yresp, xlab='', main='Fitted distribution', ylab='density', lwd=2, ylim=c(0, 1.2))
temp = apply(mDraws.t, 2, function(x) {x = density(x)
#x$y = x$y/max(x$y)
lines(x, col='red', lwd=0.6)
})
...
hist(ivResp, prob=T, ylim=c(0, 0.8))
## t samples
temp = apply(mDraws.t, 2, function(x) {x = density(x)
#x$y = x$y/max(x$y)
lines(x, col='red', lwd=0.6)
