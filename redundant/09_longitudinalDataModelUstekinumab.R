# Name: 09_longitudinalDataModelUstekinumab.R
# Auth: umar.niazi@kcl.ac.uk
# Date: 24/11/2017
# Desc: imports the longitudinal data and fits a model for treatment 2
...
dfData = read.csv('dataExternal/healthyData/diseasedDataUstekinumab.csv', header=T)
...
write.csv(dfResults, file='results/longitudinalDataResults_week1VSbaseline_Ustekinumab.csv', row.names = F)
...
dfResults = do.call(rbind, l)
dfResults$p.adj = format(p.adjust(dfResults$pvalue, method='bonf'), digi=3)
write.csv(dfResults, file='results/longitudinalDataResults_week4VSbaseline_Ustekinumab.csv', row.names = F)
...
dfResults = do.call(rbind, l)
dfResults$p.adj = format(p.adjust(dfResults$pvalue, method='bonf'), digi=3)
write.csv(dfResults, file='results/longitudinalDataResults_week12VSbaseline_Ustekinumab.csv', row.names = F)
...
dotplot(time ~ Rd.score | cells:stimulation, data=d2, panel=function(x, y, ...) panel.bwplot(x, y, pch='|', ...),
        par.strip.text=list(cex=0.6), main='Raw data 172 Groups by time in 43 Modules', xlab='RD Score')
...
dotplot(time ~ m+s1+s2 | cells:stimulation, data=d, panel=llines(d$s1, d$s2), cex=0.6, pch=20,
        par.strip.text=list(cex=0.6), main='172 Regression Coeff in 43 Modules', xlab='Model estimated Coefficients')
