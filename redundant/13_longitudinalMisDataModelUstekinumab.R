# Name: 13_longitudinalMisDataModelUstekinumab.R
...
dfData = read.csv('dataExternal/healthyData/diseasedDataMISUstekinumab.csv', header=T)
...
xyplot(Median.internalization.score ~ time | fModule, data=dfData, type=c('smooth'),
       ...
       par.strip.text=list(cex=0.6), scales = list(x=list(rot=45, cex=0.5)))

xyplot(Median.internalization.score ~ time | fModule, data=dfData, type=c('r'),
       index.cond = function(x,y) coef(lm(y ~ x))[1], #aspect='xy',# layout=c(8,2),
       par.strip.text=list(cex=0.6), scales = list(x=list(rot=45, cex=0.5)))

xyplot(Median.internalization.score ~ Visit..Week. | fModule, data=dfData, type=c('p'), pch=20,
       index.cond = function(x,y) coef(lm(y ~ x))[1], #aspect='xy',# layout=c(8,2),
       par.strip.text=list(cex=0.6), scales = list(x=list(rot=45, cex=0.5)))

dotplot(Visit..Week. ~ Median.internalization.score | fModule, data=dfData, panel=function(x, y, ...) panel.bwplot(x, y, pch='|',...), type='b',
...
table(f)
dfData$Coef = f
densityplot(dfData$Median.internalization.score, groups=dfData$Coef)
densityplot(dfData$Median.internalization.score)
...
library(lme4)
fit.lme1 = lmer(Median.internalization.score ~ 1 + (1 | Coef) + (1 | Patient.ID), data=dfData, REML=F)
summary(fit.lme1)
