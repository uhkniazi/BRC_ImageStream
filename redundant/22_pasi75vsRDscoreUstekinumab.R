# Name: 22_pasi75vsRDscoreUstekinumab.R
...
write.csv(dfData, file='dataExternal/healthyData/PASI75VsRdScoreUstekinumab.csv', row.names = F)
...
write.csv(dfResults, file='Results/PASI75vsRdScoreUstekinumab.csv', row.names = F)
...
dotplot(PASI75 ~ m+s1+s2 | cells:stimulation:time, data=d, panel=llines(d$s1, d$s2), cex=0.6, pch=20,
        par.strip.text=list(cex=0.5), main='50 Fitted Coefficients for Rd.score ~ PASI75', xlab='')
